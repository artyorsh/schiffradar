...
export class MapVM implements IMapVM, IMapPositionProvider {
  ...
  public onCameraChanged([lon, lat]: GeoJSON.Position, zoom: number): void {
    this.logger.debug(`Camera changed: ${JSON.stringify({ lon, lat, zoom }, null, 2)}`);
  ...
  public viewVesselDetails(id: number): void {
    ...
    if (!vessel) {
      this.logger.warn(`Unable to present vessel details for ${id}: vessel not found.`);
  ...
  public async getMapPosition(): Promise<GeoJSON.Position> {
    if (!this.mapPosition) {
      const currentLocation = await this.locationService.getCurrentLocation();

      return [...currentLocation, this.getDisplayZoomLevel()];
    }
  ...
  @action private onDataReceived = (data: IMapData): void => {
    ...
    this.logger.debug('Updating map data.');
    this.data = data;
  };
