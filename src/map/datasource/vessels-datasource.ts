...
export class VesselsDataSource implements IMapDataSource {
  ...
  public subscribe(positionProvider: IMapPositionProvider, callback: (data: IMapData) => void): Function {
    const pollInterval: NodeJS.Timeout = setInterval(async () => {
      try {
        const position = await positionProvider.getMapPosition();
        const vessels = await this.getVessels(position);
        callback(vessels);
      } catch(error) {
  ...
  private async getVessels(position: GeoJSON.Position): Promise<IMapData> {
    ...
    const localData = await this.localRepository.getData(tileId);
    ...
    if (isActual) {
      this.logger.debug(`[${tileId}] using local data.`);

      return mapData;
    }

    this.logger.debug(`[${tileId}] requesting remote data.`);

    const remoteData = await this.remoteRepository.getData(tileId);
